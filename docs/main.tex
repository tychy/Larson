...
\section{基礎理論}
\subsection{基礎方程式}
自己重力と放射の効果を入れた連続の式、運動方程式、エネルギー方程式を解く。
...
\begin{align}
    \dder[y]{x} &= - \frac{3}{x^4}\left(1 - \frac{1}{x}\right) + \frac{1}{x^5}\\
                &= \frac{1}{x^5}(x - 3x)
\end{align}
より$x = \frac{4}{3}$のところにある。この点は
$R$の式に代入すると、
\beq
R_m = \frac{4}{3}\tilde{R} = \frac{4\Theta}{9} \frac{G\mu M}{R_{gas} T}
\eeq
に対応する。$M = \frac{4\pi R_m^3 \rho}{3}$を代入して、
\beq
R_m = \frac{27}{16\pi\Theta} \frac{R_{gas} T}{G\mu \rho}
\eeq
クリティカルマスは$M_J$
