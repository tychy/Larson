...
def search_fion():
    ...
    # グリッドサーチ
    for rho_index in [-3, -2.5, -2, -1.5, -1, -0.5, -0.2, 0, 0.25, 0.5, 0.60]:
        rho_res = 10 ** rho_index
        tmp_ls = np.linspace(3000, 600000, 5000)
        best_low = 0.0
        best_tmp_low = 3000
        best_high = 0.0
        best_tmp_high = 3000
...
def main():
    ...
    figure = plt.figure()
    plt.plot(rho_fh, tmp_fh_low, label=r"$f_h$ = 0.1")
    plt.plot(rho_fh, tmp_fh_high, label=r"$f_h$ = 0.9")
    plt.plot(rho_fion, tmp_fion_low, label=r"$f_{ion}$ = 0.1")
    plt.plot(rho_fion, tmp_fion_high, label=r"$f_{ion}$ = 0.5")
